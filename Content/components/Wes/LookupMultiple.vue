<template>
    <select class="mt-multiselect form-control select2-lookup-multiple" :disabled="!enabled" multiple></select>
</template>
<style>
    .select2-lookup-multiple + span > span > span.select2-selection--multiple:before {
        content: "";
        position: absolute;
        right: 7px;
        top: 42%;
    ...
    .select2-lookup-multiple + span > span > span > ul > span.select2-selection__clear {
        margin-right: 20px;
    }

    .select2-lookup-multiple + span > span > span > ul > li > input.select2-search__field {
        width: 200% !important;
    }
</style>
<script>
    ...
    methods: {
        ...
        initSelect2: function() {
            ...
            .then(response => {
                ...
                window.$(vm.$el)
                .on("change", function() {
                    var eventoAEmitir = this.options ? "change" : "clear";
                    var result = [];
                    var opt;
