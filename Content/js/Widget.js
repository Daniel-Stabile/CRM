...
Widget.adjustCommandsBar = function (commandsBar, fadeIn, titleWidth) {
    // descobrir a area útil
    var usefulWidth = Widget.calulateUsefulWidth(commandsBar, titleWidth);
    var commandButtonList = commandsBar.find('.custom-action');
    var hideNestedMenu = true;

    Widget.resizePredefinedButtons(commandsBar, usefulWidth, titleWidth);
    ...
    positioningTitleArea(titleWidth, commandsBar);
};
...
Widget.loadSessions = function (key, value) {
    let sessionValues = Widget.readSessions(key, null);

    if (sessionValues !== null && sessionValues !== undefined) {
        sessionStorage.setItem(key, value);
    } else {
        sessionValue.push(value);
        sessionStorage.setItem(key, value);
    }
}

Widget.removeItem = function (key, idCommands) {
    let itens = Widget.readSessions(key, null);

    if ((itens.length || 0) > 0) {
        let itensWithoutIdCommand = itens.filter(p => p.idCommand !== idCommands);

        Widget.loadSessions(key, JSON.stringify(itensWithoutIdCommand));
    }
}

Widget.readSessions = function (key, idCommands) {
    let retorno = [];
    retorno = sessionStorage.getItem(key);
    if (retorno === null || retorno === undefined) {
        return retorno = [];
    } else {
        if (idCommands !== null && idCommands !== undefined) {
            return JSON.parse(retorno).filter(obj => obj.idCommand === idCommands);
        } else {
...
Widget.resizePredefinedButtons = function (commandsBar, usefulWidth, titleWidth) {
    let predefinedButtons = commandsBar.find('.predef-action');
    let idCommand = commandsBar.attr("id");
    let originPredefsFilter = originPredefs.filter(obj => obj.idCommand === idCommand);

    if ((titleWidth || 0) > 0) {
        if (usefulWidth < titleWidth) {

            // Retira os textos dos botões.
            if ((originPredefsFilter.length || 0) == 0) {
                for (var n = 0; n < predefinedButtons.length; n++) {
                    let textHtml = predefinedButtons[n].innerHTML;
                    let textContent = predefinedButtons[n].textContent;
                    ...
                    originPredefsFilter.push(predef);

                    if (textContent.trim() != '') {
                        predefinedButtons[n].innerHTML = textHtml.replace(textContent, "").trim();
                    }

                }
            }
            if (originPredefsFilter != null && originPredefsFilter !== undefined) {
                Widget.loadSessions("originPredefs", JSON.stringify(originPredefsFilter));
            }
        } else {
            originPredefsFilter = Widget.readSessions('originPredefs', idCommand);
            if (originPredefsFilter.length > 0) {
                var usefulWidth = ((commandsBar.width() - (titleWidth || 0)) - 20); /*margem de segurança*/
                let predefWidthOrigin = Widget.readSessions('intPredefWidthOrigin', idCommand);
                usefulWidth -= (predefWidthOrigin[0].predefWidth || 0);
                usefulWidth -= (commandsBar.find('.nested-menu').outerWidth(true) || 0) + 4;

                if (usefulWidth > titleWidth) {
                    // Retira os textos dos botões.
                    for (var n = 0; n < predefinedButtons.length; n++) {
                        if (originPredefsFilter[n].innerHTML != '') {
                            predefinedButtons[n].innerHTML = originPredefsFilter[n].innerHTML;
                        }
                    }
                    if ((predefWidthOrigin[0].predefWidth || 0) > 0)
                        Widget.removeItem("intPredefWidthOrigin", idCommand);

                    Widget.removeItem("originPredefs", idCommand);
                }
            }
        }
    }
...
Widget.calulatePredefWidth = function (commandsBar) {
    ...
    if (predefWidthOrigin.length === 0) {
        sessionValue.push({
            "predefWidth": (predefWidth || 0),
            "idCommand": idCommand
        });
        Widget.loadSessions("intPredefWidthOrigin", JSON.stringify(sessionValue));
    }

    return (predefWidth || 0);
}

Widget.calulateUsefulWidth = function (commandsBar, titleWidth) {
    var predefWidth = Widget.calulatePredefWidth(commandsBar);

    // descobrir a área útil
    var usefulWidth = ((commandsBar.width() - (titleWidth || 0)) - 20); /*margem de segurança*/
    usefulWidth -= (predefWidth || 0);
    usefulWidth -= (commandsBar.find('.nested-menu').outerWidth(true) || 0) + 4;
...
Widget.adjustCommandsBarForAllWidgets = function (fadeIn) {
    ...
    for (var i = 0; i < commandsBarList.length; i++) {
        ...
        var idCommandsBar = $(commandsBarList[i]).attr("id");
        var titleWidth = 0;

        if (typeof idCommandsBar !== 'undefined') {
            var idDivPai = idCommandsBar.replace("_toolbar", "");
            var title = $(commandsBarList[i]).closest("div[id=" + idDivPai + "]").find(".portlet-title.top-actions .caption")
        ...
        // disparar apenas se a diferença com tela anterior for maior que 10px
        var lastWidth = commandsBar.data("lastWidth");
        if (!lastWidth)
            lastWidth = 0;

        Widget.adjustCommandsBar(commandsBar, fadeIn, titleWidth);
...
Widget.init = function () {
    sessionStorage.removeItem("originPredefs");
    sessionStorage.removeItem("intPredefWidthOrigin");
...
function positioningTitleArea(titleWidth, commandsBar) {
    const actions = commandsBar.find('a:visible');
    let actionsWidth = 0;

    actions.each(function () {
        actionsWidth += $(this).outerWidth(true) + 3;
    });

    const widgetArea = commandsBar.parents(".widget");
    if (!widgetArea.length) return;

    const rect = widgetArea[0].getBoundingClientRect();
    const usefulArea = rect.width - actionsWidth;

    if (usefulArea < titleWidth) {
        const titleArea = commandsBar.parents(".portlet").find('.portlet-title.top-actions');
        if (titleArea.length) {
            titleArea.css('display', 'contents');
        }
    }
}
