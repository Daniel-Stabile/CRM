...
class GraphicXY extends GraphicBase {
    ...
    getChart(id, objDefinition) {
        ...
        this.applyResponsivityRules(chart);
    ...
    applyResponsivityRules(chart) {
        this.EnableDefaultResponsivityRulesAmcharts(chart);

        var graphic = this;
        chart.responsive.rules.push({
            relevant: function (target) {
                graphic.yAxisLabelResponsivityRule(target)
                graphic.xAxisLabelResponsivityRule(target)
                return true;
            },
    ...
    EnableDefaultResponsivityRulesAmcharts(chart) {
        chart.responsive.enabled = true;
        chart.responsive.useDefault = true;
    }

    yAxisLabelResponsivityRule(target) {
        if (!(target instanceof am4charts.Chart) || target.yAxes == null || target.yAxes.getIndex(0) == null) {
            return;
        }

        var label = target.yAxes.getIndex(0).renderer.labels.template;

        if (target.pixelWidth <= 200) {
            label.truncate = true;
            label.maxWidth = 50;
        }
        else if (target.pixelWidth <= 600) {
            label.truncate = true;
            label.maxWidth = 100;
        }
        else if (target.pixelWidth <= 800) {
            label.truncate = true;
            label.maxWidth = 200;
        }
        else {
    ...
    xAxisLabelResponsivityRule(target) {
        if (!(target instanceof am4charts.Chart) || target.xAxes == null || target.xAxes.getIndex(0) == null) {
            return;
        }

        var label = target.xAxes.getIndex(0).renderer.labels.template;
        label.wrap = true;
        label.maxWidth = 100;
    }
