...
var Search = function () {
            ...
            $('select.benner-search.select2-hidden-accessible').each(function () {
                ...
                $(this).on('select2:unselecting', function (e) {
                    ...
                    var ro = new ResizeObserver(entries => {
                        ...
                        if (element.width() < 210) {
                            element.width(210);
                        }
                    });

                    var container = $('.select2-container--bootstrap .select2-dropdown')[0];
                    if (container) {
    ...
    var doPostBack = function (lookup) {
        var url = $(lookup).attr('postback-url');
        if (url !== null)
            eval(url);
    }

    var validateSelectionLimit = function (control, event) {
        if (!isMultiple(control)) {
            return;
        }

        var maxItems = control.data("maxitems");
        var selectedItems = control.select2("data");

        if (selectedItems != null && maxItems != null && selectedItems.length >= maxItems) {
            event.preventDefault();
            alert('Você pode selecionar até ' + maxItems + ' itens.');
        }
    }

    var isMultiple = function (control) {
        return control.prop('multiple');
    }
    ...
    if (control.attr("multiple") === "multiple") {
        ...
        if (typeOperation === 1) {
            ...
            selectedVal = JSON.stringify({
                Readonly: false,
                View: false,
                SelectedItems: [{ id: valueControl, text: control.find(":selected").text() }]
            });
        }
    }
...
Search.createButtonToRegister = function (searchControl, typeOperation) {
    if ($(searchControl).data("url") !== "" && $(searchControl).data("canurl") === "True") {
        ...
        if (typeOperation === 0) {
            btnKeepRecord.children().attr("class", "btn btn-secondary");
