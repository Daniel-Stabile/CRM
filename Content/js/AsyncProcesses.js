...
var AsyncProcesses = function () {
    ...
    var getProcessCssClass = function () {
        switch (this.Status) {
            case 0:
    ...
    var showErrorMessage = function (detail) {
        ...
        html.append('<p class="break-lines">' + detail.ErrorMessage + '</p>');
        if (detail.Url != null) {
            html.append('<p></p>');
            html.append('<p></p>');
    ...
    return {
        ...
        loadAsyncProcesses: function (result) {
            ...
            for (var i = result.length - 1; i >= 0; i--) {
                ...
                detail.getProcessCssClass = getProcessCssClass;
                ...
                if ((detail.WithoutProgressBar || detail.Status == 4 || detail.Status == 5) && detail.Progress != null)
                {
                    var progres = String.format('<i class="{0}">{1}</i>', detail.getProcessCssClass(), detail.Progress);
                    description = String.format('{0}<br>{1}', detail.Description, progres);
                }
                ...
                sb.append(String.format('      <span class="desc"><i class="{0} {1}"></i> {2}</span>', detail.getProcessClass(), detail.getProcessCssClass(), description));
                ...
                if (!detail.WithoutProgressBar && detail.Status != 4 && detail.Status != 5) {
                ...
                if (detail.Status != 3 && detail.Status != 5 &&  !detail.ToastrDisplayed) {
