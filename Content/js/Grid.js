...
Grid.updateTotalizerRow = function (widgetClientId, isSimpleGrid, isSelectedAllRows, widgetId, selectedHandle) {
    ...
    if (selectedHandlesList.length == 0 && selectedHandle == null) {
        totalizeInputValues = null;
    }
...
function setExtraOptionsPosition(widgetId) {
    var updatePanel = $("#" + widgetId + " .update-panel");
    var extraOptionsButton = $("#" + widgetId + " .extra-options-button");
    var extraOptionsList = $("#" + widgetId + " .extra-options-list");

    var containerOffset = updatePanel.offset();
    var buttonOffset = extraOptionsButton.offset();
    var iconOffset = extraOptionsButton.find('i').offset();
    var containerPosition = updatePanel.position();

    var buttonTopRelativeToContainer = buttonOffset.top - containerOffset.top;
    var iconLeftRelativeToContainer = iconOffset.left - containerOffset.left;

    var listWidth = extraOptionsList.width();
    var magicNumberPxLeft = 47;
    if (containerPosition.top == 0 && containerPosition.left == 0) {
        magicNumberPxLeft = 27;
    }

    var listLeft = iconLeftRelativeToContainer - listWidth + magicNumberPxLeft;
    extraOptionsList.css('left', listLeft + 'px');

    var buttonHeight = extraOptionsButton.height();
    var listTop = buttonTopRelativeToContainer + buttonHeight + containerPosition.top;
    extraOptionsList.css('top', listTop + 'px');
}

function configureExtraOptions(widgetId) {
    var extraOptionsButton = $("#" + widgetId + " .extra-options-button");
    var extraOptionsButtonExists = extraOptionsButton.length > 0;
    if (extraOptionsButtonExists) {
        var extraOptionsList = $("#" + widgetId + " .extra-options-list");

        extraOptionsButton.off('mouseenter mouseleave');
        extraOptionsList.off('mouseenter mouseleave');

        extraOptionsButton.on('mouseenter', function () {
            setExtraOptionsPosition(widgetId);
            extraOptionsList.show();
        });

        extraOptionsButton.on('mouseleave', function () {
            var t = setTimeout(function () {
                extraOptionsList.hide();
            }, 100);
            
            extraOptionsList.on('mouseenter', function () {
                clearTimeout(t);
                extraOptionsList.show();
            })

            extraOptionsList.on('mouseleave', function () {
                extraOptionsList.hide();
            });
        });
    }
...
Grid.initExtraOptions = function () {
    _.forEach(Grid.widgets, (element) => {
        configureExtraOptions(element.name);

        var mutationObserver = new MutationObserver(function (e) {
            configureExtraOptions(element.name);
        });

        var updatePanel = $("#" + element.name + " .update-panel");
        mutationObserver.observe(updatePanel[0], { childList: true });
    });
};
