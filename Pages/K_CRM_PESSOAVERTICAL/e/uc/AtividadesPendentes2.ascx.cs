...
public partial class Pages_K_CRM_PESSOAVERTICAL_e_AtividadesPendentes : UserControlBase<EntityBase>
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if (entidadeBase != null)
        {
            ...
            string handle = (entidadeBase.Fields["PESSOA"] as EntityAssociation).Instance.Handle.ToString();
            string vert = (entidadeBase.Fields["VERTICAL"] as EntityAssociation).Instance.Handle.ToString();
            ...
            query.CommandText.AppendLine(" WHERE A.PESSOA = :HANDLE AND A.VERTICAL = :VERTICAL AND A.STATUS = 1");
            query.CommandText.AppendLine(" ORDER BY A.DATAAGENDAMENTO ASC");

            query.Parameters.Add("HANDLE", Convert.ToInt32(handle));
            query.Parameters.Add("VERTICAL", Convert.ToInt32(vert));
            ...
            var outPut = new FormLinkDefinition
            {
                ...
                WhereClause = new WhereClause("A.PESSOA = " + handle + " AND A.USUARIO = " + BennerIdentity.Current.UserHandle + " AND A.VERTICAL = " + vert) 
